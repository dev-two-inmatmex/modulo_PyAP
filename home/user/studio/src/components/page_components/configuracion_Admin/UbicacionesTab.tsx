...
const MapaEdicion = dynamic(() => import('@/app/(roles)/administracion/configuraciones/components/MapaEdicion'), { 
  ssr: false, 
...
export function UbicacionesTab({ ubicacionInicial, onFinished }: UbicacionesTabProps) {
  ...
  const [nombre, setNombre] = useState(ubicacionInicial?.nombre ?? DEFAULT_LOCATION.nombre);
